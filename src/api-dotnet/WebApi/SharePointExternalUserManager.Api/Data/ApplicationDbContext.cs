...
public class ApplicationDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<TenantUserEntity>(entity =>
        {
            ...
            // Unique constraint: one user can have only one role per tenant
            entity.HasIndex(e => new { e.TenantId, e.UserId })
                .IsUnique()
                .HasDatabaseName("UQ_TenantUsers_TenantId_UserId");

            // Index on UserId for user lookup across tenants
            entity.HasIndex(e => e.UserId)
                .HasDatabaseName("IX_TenantUsers_UserId");

            // Index on Email for email-based searches
